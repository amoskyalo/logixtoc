...
const DeliveryPlanTable = ({ loading, rows }: DeliveryPlanTableInterface) => {
    ...
    const columns: GridColDef[] = [
        ...
        },
        {
            ...
            headerName: 'Status',
            ...(isMobile ? { width: 150 } : { flex: 1 }),
            renderCell: (param) => <StatusChips statusID={param.row.DeliveryPlanStatusID} name={param.row.DeliveryPlanStatusName} />,
        },
    ];

    const renderActionButton = () => {
        return <Chip label="View all" color="secondary" onClick={() => router.push('/dashboard/inventory/planning')} sx={{ width: 75 }} />;
    };
