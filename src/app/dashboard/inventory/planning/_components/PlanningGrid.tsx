...
import { DatesInterface } from "@/components/DataGrids/DataGridToolbar";
...
const PlanningGrid = ({ rows, isLoading, dates, setDates }: PropsInterface) => {
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 150,
      renderCell: ({
        row: { DeliveryPlanStatusName, DeliveryPlanStatusID },
      }) => {
        return (
          <StatusChips
            name={DeliveryPlanStatusName}
            statusID={DeliveryPlanStatusID}
          />
        );
      },
    },
    {
      ...
      type: "actions",
      getActions: () => {
        return [<DataGridEditNDelete />];
      },
    },
  ];

  const toolbar = () => (
    <DataGridToolbar dates={dates} setDates={setDates} onAdd={() => null} />
  );
