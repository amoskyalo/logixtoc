...
import { ProductType, useMutate } from '@/api';
...
const ProductTypeGrid = ({ rows, isLoading, refetch, ...otherProps }: GridProps<ProductType>) => {
    ...
    const { mutate } = useMutate<{ vendorProductTypeID: number }>('deleteVendorProductTypes');
    ...
    const handleDelete = () => {
        if (activeRow.row) {
            ...
            mutate({ vendorProductTypeID: activeRow.row.VendorProductTypeID }, mutateOptions({ setLoading, refetch, onClose }));
        }
    };
    ...
    return (
        <>
            ...
            <DeleteDialog open={activeRow.type === 'delete'} loading={loading} onCancel={onClose} onOkay={handleDelete} />

            <UOMList rows={activeRow?.row?.VendorProductTypeUOM || []} open={activeRow.type === 'uom list'} onClose={onClose} />

            <ClassList rows={activeRow?.row?.VendorProductTypeClass || []} open={activeRow.type === 'class list'} onClose={onClose} />

            <BrandsList rows={activeRow?.row?.VendorProductTypeBrand || []} open={activeRow.type === 'brands list'} onClose={onClose} />
        </>
    );
};
