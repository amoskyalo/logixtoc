...
const AssignedLocationForm = ({
  ...
  const validationSchema = () => {
    return Yup.object().shape({
      vendorLocationID: Yup.number().required(
        "Atleast one location is required"
      ),
      locationsArray: Yup.array().min(
        1,
        "Atleast one assigned location should be selected"
      ),
    });
  };
            ...
            <Form>
              <Stack spacing={3}>
                ...
                <AutoCompleteField
                  ...
                  error={
                    touched.locationsArray && Boolean(errors.locationsArray)
                  }
                  helperText={touched.locationsArray && errors.locationsArray}
                />
