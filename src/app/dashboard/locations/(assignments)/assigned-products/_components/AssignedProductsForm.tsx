...
const AssignedProductsForm = ({ open, onClose, products, refetch }: FormsPropsInterface & Props) => {
                    ...
                    return (
                        <Form>
                            <Stack spacing={3}>
                                ...
                                <AutoCompleteField
                                    options={products.map(({ VendorProductTypeName, VendorProductTypeID }) => ({
                                        title: VendorProductTypeName,
                                        vendorProductTypeID: VendorProductTypeID,
                                    }))}
                                    getOptionLabel={(product: any) => product.title}
                                    label="Products"
                                    {...getFormikFieldProps(formik, 'productTypeArray', true)}
                                />
